...
function useUpdateApi<I, O>(resourceUrl: string, resourceKey: string, entityId: number, isAdmin = true) {
  const queryClient = useQueryClient();

  return useMutation<O, ErrorMessage, I>(
    (requestBody) => FetchUtils.update<I, O>(resourceUrl, entityId, requestBody, isAdmin),
    {
      onSuccess: () => {
